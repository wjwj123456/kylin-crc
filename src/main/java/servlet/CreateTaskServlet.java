...
public class CreateTaskServlet extends HttpServlet {
	...
	private void createNewTask(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		int result = review.saveReviewInfo(new TaskVO(userName, jsonObject.getString("taskName"),
				Type.valueOf(jsonObject.getString("type")), "", jsonObject.getString("describe"), deadline, 0,
				Language.valueOf(jsonObject.getString("language")), Power.valueOf(jsonObject.getString("power"))));

		PrintWriter out = response.getWriter();
		out.print(result);
	}

	private void changeTime(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		ReviewBlImpl review = new ReviewBlImpl();
		int result = review.setDeadline(taskName, deadline);
